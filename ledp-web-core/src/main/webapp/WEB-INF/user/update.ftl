...
<#macro content>
	...
	<div class="floatL detaillist">
		<form id="form_user_update" class="validateCheck" action="${rc.contextPath}/user/edit" method="post" data-options="novalidate:true">
			<#include "/user/_form.ftl"/>
			<input type="hidden" name="id" value="${user.id}"/>
		</form>
		<div class="btnsumbit">
			<a href="javascript:void(0)" class="easyui-linkbutton btnstyle" onClick="$('#form_user_update').submit();">保 存</a>
