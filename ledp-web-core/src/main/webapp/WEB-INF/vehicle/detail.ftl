	...
	<div class="detaillist ollright">
		<ul class="formlist">
			<li class="clearfix">
				...
				<span class="iname">${vehicle.series.name?if_exists}</span>
			</li>
			<li class="clearfix">
				<label>车型名称<em>*</em></label>
				<span class="iname">${vehicle.name?if_exists}</span>
			</li>
			<li class="clearfix">
				<label>车型代码<em>*</em></label>
				<span class="iname">${vehicle.code?if_exists}</span>
			</li>
			<li class="clearfix">
				<label>车型状态<em>*</em></label>
				<span class="iname">
				${vehicle.status.name?if_exists}
				</span>
			</li>
			<li class="clearfix">
				<label>在售状态<em>*</em></label>
				<span class="iname">
			...
			<li class="clearfix">
				...
				<span class="iname">${vehicle.price?if_exists}</span>
			</li>
		</ul>
		<div class="btnsumbit btncoloes">
			<#if loginDealer?exists>
				<a href="${rc.contextPath}/vehicle/editPrice/${vehicle.id}" class="easyui-linkbutton btnstyle" onclick="submitForm()">编 辑</a>
			<#else>
				<a href="${rc.contextPath}/vehicle/update/${vehicle.id}" class="easyui-linkbutton btnstyle" onclick="submitForm()">编 辑</a>
			</#if>
			<a href="${rc.contextPath}/vehicle/index" class="easyui-linkbutton backbtn">返 回</a>
		</div>
	</div>

<table id="vehicleParamCategoryDg" style="width:700px;height:250px">
</table>
<table id="vehicleImageDg" style="width:700px;height:250px">
</table>
...
<script type="text/javascript" charset="utf-8">
    ...
    (function(global,$) {
        ...
        VehicleParam.init = function() {
            var id=$("#_id0").val();
            $('#vehicleImageDg').datagrid({
                title : '图片',
                idField : 'id',
                singleSelect:true,
                fitColumns:true,
                columns:[[
                    ...
                    {field:'path',title:'物理地址'},
                    {field:'webpath',title:'网络访问地址'},
                    {field:'status',title:'状态'}
                ]],
                url : '${rc.contextPath}/vehicleImage/get/'+id
            });
            $('#vehicleParamCategoryDg').datagrid({
				view : detailview,
                title : '参数',
                idField : 'id',
				singleSelect:true,
                ...
                columns:[[
                    {field:'id',hidden:true},
                    {field:'name',title:'分类名称',width:800},
                    {field:'weight',hidden:true}
                ]],
                url : '${rc.contextPath}/vehicleParamCategory/get/'+id,
                detailFormatter:function(index,row){
                ...
                onExpandRow: function(index,row){
                    ...
                    ddv.datagrid({
                        ...
                        url : '${rc.contextPath}/vehicleParam/get/'+row.id,
                        onResize:function(){
                            $('#vehicleParamCategoryDg').datagrid('fixDetailRowHeight',index);
                        },
        ...
        global.VehicleParam = VehicleParam;
    })(this,jQuery);
