...
<form id="autoReplyForm" action="${rc.contextPath}/wechat/autoreply/message" enctype="multipart/form-data" method="POST">
	<input name="type" value="message" type="hidden"/>
	<input name="status" value="${autoReply.status?default('0')}" type="hidden"/>
	<input name="msgType" id="msgType" value="${autoReply.msgType?default('news')}" type="hidden"/>
	<input name="materialId" id="materialId" value="${autoReply.materialId}" type="hidden"/>
    <div class="tabContainer autoReply" style="border: 0px; width:96%;margin-top: 10px;">
        <ul class="tabContiner_nav">
            <li href="#" onClick="changeType('subscribe')">订阅自动回复</li>
            <li href="#tab_contianer" class="active">消息自动回复</li>
        </ul>
		...
		$("input[name='msgType']").val("news");
     	...
     	if(data.type.id=='9003'){
	     	if(data.children==undefined||data.children.length==0){
                ...
                news+="				<a href='${rc.contextPath}/wechat/material/edit/"+data["id"]+"'>";
    ...
    function removeNews(){
    	layer.confirm("确定清除内容吗？",{},
			function(index){
				...
				news_continer+="			<input type='file' style='width:68px;' name='image' id='image_tab' class='floatL' />";
				news_continer+="		</a>";
				news_continer+="	</div>";
				news_continer+="</div>";
				news_continer+="<div class='addPi1'>";
