...
<div class="breadcrumbs" id="breadcrumbs">
	...
	<ul class="breadcrumb">
		<li>
			...
			<a href="#">营销应用</a>
			<span>&gt;&gt;</span>
			<#if site.campaign?exists>
				<span><a href="${rc.contextPath}/wechat/campaign/index">活动管理</a></span>
				<span>&gt;&gt;</span>
				<span><a href="${rc.contextPath}/wechat/campaign/update/${site.campaign.id}">填写活动基本信息</a></span>
				<span>&gt;&gt;</span>
				<span><a href="${rc.contextPath}/wechat/campaign/callBack?campaignId=${site.campaign.id}">选择模板</a></span>
			<#else>
...
<div class="active_slider">
	...
	<div class="sliderPointer marginLeft15">
		...
		<span>填写<#if site.campaign?exists>活动<#else>微站</#if>基本信息</span>
	</div>
...
<div class="activemainPages">
	...
	<div class="clearFix" style="padding-left: 380px;">
		...
		<#if site.campaign?exists>
		<input type="button" class="tongyiButton" style="width:100px;float:left;margin-left:20px;"  onclick="window.location.href='${rc.contextPath}/wechat/campaign/callBack?campaignId=${site.campaign.id}'" value="返回" />
		<#else>
		<input type="button" class="tongyiButton"  style="width:100px;float:left;margin-left:20px;" onclick="window.location.href='${rc.contextPath}/wechat/campaign/callBack?siteId=${site.id}'" value="返回" />
		</#if>
	</div>
</div>
</#macro>
<#macro script>
	...
	<script>
		...
		$(function(){
			...
			if(name==""){
				...
				if(name!=$sitPageName){
					...
					$.ajax({
						url:"${rc.contextPath}/wechat/campaign/saveNewPageNmae",
		...
		function daileSitePage(){
			var id=$('#sitePageId').val();
			if(id){
				var reqUrl = "${rc.contextPath}/wechat/campaign/getUrl/"+id+"?r="+new Date().getTime();
