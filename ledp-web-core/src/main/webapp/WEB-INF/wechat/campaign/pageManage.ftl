...
<h1 class="title_page">活动中心 >> ${campaign.name} >> 界面列表</h1>
<form action="${rc.contextPath}/wechat/campaign/searchSitePage" class="form-search" method="post" id="formId">
	<intput type="hidden" id="campaignId" name="campaignId" class="search" value="${campaign.id}"></input>
</form>
<div id="tableId">
...
<form id="form_camp" action="${rc.contextPath}/wechat/campaign/pageUpdate" method="post" >
	<intput type="hidden" name="campaign"  value="${campaign.id}"></input>
	<intput type="hidden" id="sitePageId" name="sitePageId"  ></input>
</form>
</#macro>
<#macro script>
	...
	var page = new pagination("formId","tableId",function(value){
		...
		if(value.length>0){
			for(var i=0;i<value.length;i++){
				...
				if(value[i]["end"]){
					if(value[i]["dealer"]){
						str+="<a href='${rc.contextPath}/wechat/campaign/pageUpdate?campaign=${campaign.id}&sitePageId="+value[i]["id"]+"' ><i class='icon-edit' title='编辑'></i></a>";
...
function daileSitePage(id){
	...
	var layerPage = new ElasticLayer("pageWindow",reqUrl,{area:["400px","80%"],offset:["5%","40%"]},{
