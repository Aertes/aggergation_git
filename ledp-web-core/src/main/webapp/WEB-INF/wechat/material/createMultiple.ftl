				...
				<div class="tuwenNewDiv">
					...
					<div class="floatL tuwenNew" id="pictureForId0">
						<div class="tuwen_option">
							<label>标题：</label>
							<input type="text" id="title" placeholder="请输入1-64长度的标题" maxlength="64" />
							<span class='messageCheckP' style='margin-left:45px;'></span>
						</div>
						<div class="tuwen_option">
							<label>作者：</label>
							<input type="text" id="author" placeholder="请输入1-8长度的作者名称" maxlength="8"/>
							<span class='messageCheckP' style='margin-left:45px;'></span>
						</div>
						<div class="tuwen_option">
							<label>封面：</label>
							<form id="materialForm" method="post" enctype="multipart/form-data">
								<input type="file" id="upload_image" style="line-height:20px;height:20px;" name="image" value="上传" />
						...
						<div class="tuwen_option">
							...
							<textarea id="editor_id" style="width:670px;height:300px;"  maxlength="20000"></textarea>
							<span class='messageCheckP'></span>
						</div>
						<div class="tuwen_option">
							<label>原文链接（选填）</label>
							<input type="text"  placeholder="请输入http://或https://开头的链接" id="originalUrl" />
...
<script type="text/javascript" src="${rc.contextPath}/wechat/js/report/highcharts.js" ></script>
		...
		<!--
		<script charset="utf-8" src="${rc.contextPath }/wechat/js/kindeditor/kindeditor.js"></script>
		-->
		<script>
		//总图文数
		var total = 1;
		...
		$(function(){
			...
			$("a[class='icon-plus']",$(".addLotTu")).click(function(){
				...
				var $content = $("<input type='hidden' checkdata='notNull' checkName='title' id='title"+addIndex+"'/><input type='hidden' checkName='author' id='author"+addIndex+"'/><input type='hidden' checkName='showCoverPic' id='showCoverPic"+addIndex+"'/><input checkName='digest' type='hidden'  id='digest"+addIndex+"'/><input type='hidden'  checkdata='notNull' checkName='content'  id='content"+addIndex+"'/><input type='hidden' checkName='originalUrl' id='originalUrl"+addIndex+"'/><input type='hidden' checkdata='notNull' checkName='thumbMediaId'  id='thumbMediaId"+addIndex+"'/><input type='hidden' checkName='url'  id='url"+addIndex+"'/><input type='hidden' checkName='filepath'  id='filepath"+addIndex+"'/><label class='marginTop20 subtitle'>标题</label><div class='animatePicture_chil'><img width='100px' height='110px' src=''/></div>");
       ...
       function cancel() {
		...
		function saveAndMass(){
			...
			if(isRight){
				...
				setJson();
				$("#multipleForm").attr("action","${rc.contextPath}/wechat/material/saveMultipleAndMass");
				$("#multipleForm").submit();
			}
		}
