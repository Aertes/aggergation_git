				...
				<div class="tuwenNewDiv">
					<div class="floatL tuwenResult addLotTu" >
						<div class="position-relative tutu pictureCheck dataitem" index=0 id="item0" >
							...
							<input type="hidden" checkdata="originalUrl" checkName="originalUrl" id="originalUrl0" value="${material.originalUrl?if_exists}"/>
					...
					<div class="floatL tuwenNew" id="pictureForId0">
						...
						<div class="tuwen_option">
							<label>原文链接（选填）</label>
							<input type="text"  placeholder="请输入http://或https://开头的链接"  checkdata="originalUrl" checkName="originalUrl"  value="${material.originalUrl?if_exists}" checkdata="originalUrl" id="originalUrl" />
...
<script>
		...
		$(function(){
			...
			$("a[class='icon-plus']",$(".addLotTu")).click(function(){
				...
				var $shadow = $("<div class='shadow'><a class='icon-edit'  style='width: 100%;' title='编辑'></a></div>");
		...
		function save(){
			...
			if(isRight){
				...
				setJson();
				$("#multipleForm").attr("action","${rc.contextPath}/wechat/material/updateAndRedirect");
				$("#multipleForm").submit();
			}
		}
