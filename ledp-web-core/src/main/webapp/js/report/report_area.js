$(document).ready(function(){
	...
	initWeekPhonePieReport();	//400来电 - 本周接起率
...
function searchReport(){
	var searchRegion = $("#searchRegion").combo("getValues");
	if(!searchRegion || searchRegion.length == 0){
		alert("查询大区不能为空");
		return;
	}
	
	if(searchRegion.length != 1){
		alert("查询大区数量不能超过1个");
		return;
	}
	
	window.location.href = "index?searchRegion=" + searchRegion;
