...
(function () {
        ...
        dialog.onok = function () {
            ...
            switch (id) {
                case 'upload':
                    ...
                    editor.execCommand('insertHtml', list);
                    break;
                case 'online':
    ...
    UploadFile.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
            	...
            	data.keywardLabel = otherKey;
            	if(typeof window.parent.resourceCategoryId != undefined){
            		data.resourceCategoryId = window.parent.resourceCategoryId;
            	}
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < this.fileList.length; i++) {
               ...
               /* list.push({
                ...
                src = data.playerUrl+data.url;
                title= data.title;
                var suffix=data.url.substr(data.url.lastIndexOf('.')+1);
                var s=window.parent.ueditURL;
                if(suffix=='pdf'){
                	list = list+ '<p style="line-height: 16px;"><img style="vertical-align: middle; margin-right: 2px;" src="'+s+'dialogs/attachment/fileTypeImages/icon_pdf.gif" _src="'+s+'dialogs/attachmentfileTypeImages/icon_pdf.gif"><a style="font-size:12px; color:#0066cc;" href="'+data.playerUrl+'/resource/downloadPdf?id='+data.uuid+'&name='+title+'&suffix='+suffix+'" title="'+title+'" _href="">'+title+'</a></p>'
                }else{
                	list = list+ '<p style="line-height: 16px;"><img style="vertical-align: middle; margin-right: 2px;" src="'+s+'dialogs/attachment/fileTypeImages/icon_rar.gif" _src="'+s+'dialogs/attachmentfileTypeImages/icon_pdf.gif"><a style="font-size:12px; color:#0066cc;" href="'+data.playerUrl+'/resource/downloadPdf?id='+data.uuid+'&name='+title+'&suffix='+suffix+'" title="'+title+'" _href="">'+title+'</a></p>'
                }
            }
            return list;
        }
    ...
    SearchImage.prototype = {
        ...
        getImageData: function(){
          ...
          $.ajax({
           	url: '/jyedu/resource/resouceSearch',
          	data: data,
           	type:"post"
          	}).done(function (json) {
            	if(json && json[0]) {
                    for(var i = 0; i < json[0].length; i++) {
                        if(json[0][i]) {
                            list.push({
                                title: json[0][i].name,
                                src: json[0][i].imageUrl,
                                url: json[0][i].webPath,
                                id:json[0][i].id,
                                scrUrl:json[0][i].url
                            });
                        }
                    ...
                    _this.setList(list);
                    var pages = json[1];
                    var activePage = json[2];
                    if(pages > 0){
                    	var $div = $('<div>');
                    	var $shangyiye = $('<a><</a>');
                    	$shangyiye.click(function(){
                    		 var $prev = $(this).parnet().find('.active').prev();
                    		 if($prev.length){
                    			 chaxun($prev.text())
                    		 }
                    	});
                        ...
                        $xiaye.click(function(){
        	           		 var $next = $(this).parnet().find('.active').next();
        	           		 if($next.length){
        	           			 chaxun($next.text())
        	           		 }
                        });
                        $div.append($shangyiye)
...
//            });
         /*   
            var json = {
            		data : [{
            			title: 'aaa',
                        src: 'http://ueditor.baidu.com/server/ueditor/upload/image/4%202.jpg?noCache=i3uxztks',
                        url: 'http://ueditor.baidu.com/server/ueditor/upload/image/4%202.jpg?noCache=i3uxztks'
            		},{
            			title: 'aaa',
                        src: 'http://ueditor.baidu.com/server/ueditor/upload/image/4%202.jpg?noCache=i3uxztks',
                        url: 'http://ueditor.baidu.com/server/ueditor/upload/image/4%202.jpg?noCache=i3uxztks'
...
//            });
        ...
        setList: function (list) {
            ...
            if(list.length) {
                for (i = 0; i < list.length; i++) {
                    ...
                    img.width = 113;
                    var suffix=list[i].url.substr(list[i].url.lastIndexOf('.')+1);
                    if(suffix=='pdf'){
        ...
        getInsertList: function () {
            ...
            for(var i = 0; i < items.length; i++) {
                ...
                if(child.tagName && child.tagName.toLowerCase() == 'img' && domUtils.hasClass(items[i], 'selected')) {
                    ...
                    var s=window.parent.ueditURL;
                    if(suffix=='pdf'){
                    	list = list+ '<p style="line-height: 16px;"><img style="vertical-align: middle; margin-right: 2px;" src="'+s+'dialogs/attachment/fileTypeImages/icon_pdf.gif" _src="'+s+'dialogs/attachmentfileTypeImages/icon_pdf.gif"><a style="font-size:12px; color:#0066cc;" href="'+a_1.name+'/resource/downloadPdf?id='+a_1.id+'&name='+title+'&suffix='+suffix+'" title="'+title+'" _href="">'+title+'</a></p>'
                    }else{
                    	list = list+ '<p style="line-height: 16px;"><img style="vertical-align: middle; margin-right: 2px;" src="'+s+'dialogs/attachment/fileTypeImages/icon_rar.gif" _src="'+s+'dialogs/attachmentfileTypeImages/icon_rar.gif"><a style="font-size:12px; color:#0066cc;" href="'+a_1.name+'/resource/downloadPdf?id='+a_1.id+'&name='+title+'&suffix='+suffix+'" title="'+title+'" _href="">'+title+'</a></p>'
                    }
                    /*list.push({
                        src: src,
                        _src: src,
    ...
    function chaxun(activPage1){
    	var _this = new SearchImage();
    	var list = [];
        var data = {
        		name:$('#searchTxt').val(),
        		type:$('#type').val(),
