...
(function () {
    ...
    function setTabFocus(id) {
        ...
        switch (id) {
            ...
            case 'search1':
            	setAlign(editor.getOpt('imageManagerInsertAlign'));
            	searchImage1 = searchImage1 || new SearchImages();
            	break;
        }
    }
        ...
        dialog.onok = function () {
            ...
            switch (id) {
                ...
                case 'search':
                    ...
                    break;
                case 'search1':
                	list = searchImage1.getInsertList();
                	break;
            }
...
//            });
    ...
    SearchImages.prototype = {
        ...
        initEvents: function(){
            ...
            /* 点击搜索按钮 */
            domUtils.on($G('searchBtn1'), 'click', function(){
                var key = $G('searchTxt1').value;
                if(key && key != lang.searchRemind) {
                    _this.getImageData();
                }
            });
            /* 点击清除妞 */
            domUtils.on($G('searchReset1'), 'click', function(){
                $G('searchTxt1').value = lang.searchRemind;
                $G('searchListUl1').innerHTML = '';
                $G('searchType1').selectedIndex = 0;
            });
            /* 搜索框聚焦 */
            domUtils.on($G('searchTxt1'), 'focus', function(){
                var key = $G('searchTxt1').value;
                if(key && key == lang.searchRemind) {
                    $G('searchTxt1').value = '';
                }
            ...
            /* 搜索框回车键搜索 */
            domUtils.on($G('searchTxt1'), 'keydown', function(e){
                var keyCode = e.keyCode || e.which;
                if (keyCode == 13) {
                    $G('searchBtn1').click();
                }
            ...
            /* 选中图片 */
            domUtils.on($G('searchList1'), 'click', function(e){
                var target = e.target || e.srcElement,
                    li = target.parentNode.parentNode;
        ...
        getImageData: function(){
            var _this = this,
                key = $G('searchTxt1').value,
                type = $G('searchType1').value,
            ...
            $G('searchListUl1').innerHTML = lang.searchLoading;
            ajax.request(url, {
                'dataType': 'jsonp',
                'charset': 'GB18030',
                'onsuccess':function(json){
                ...
                'onerror':function(){
                    $G('searchListUl1').innerHTML = lang.searchRetry;
                }
            });
        },
        /* 添加图片到列表界面上 */
        ...
        getInsertList: function () {
            var child,
                ...
                align = getAlign(),
                list = [],
                items = $G('searchListUl1').children;
            for(var i = 0; i < items.length; i++) {
                child = items[i].firstChild && items[i].firstChild.firstChild;
                if(child.tagName && child.tagName.toLowerCase() == 'img' && domUtils.hasClass(items[i], 'selected')) {
                    src = child.src;
